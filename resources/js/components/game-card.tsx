...
const GameCard3D = (props: CardProps) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            const { cardTransform, imageTransform } = updateCardTransform(lastMousePosition.current.x, lastMousePosition.current.y);
    ...
    return (
        <Link href={props.link}>
            <Card ref={cardRef} className="group max-w-md rounded-[calc(1rem+0.5rem)] pt-2 hover:border-accent">
                <CardContent className="space-y-6 px-2 text-sm">
                    <div className="overflow-hidden rounded-2xl border">
                        <img ref={imageRef} src={props.image} alt={props.alt} className="aspect-square w-full rounded-md object-cover" />
                    </div>
                </CardContent>
                <CardHeader>
                    <CardTitle className="text-center group-hover:text-accent">{props.title}</CardTitle>
                </CardHeader>
            </Card>
        </Link>
    );
